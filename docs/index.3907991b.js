...
})({"8ov0T":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
parcelHelpers.export(exports, "default_opml", ()=>default_opml
);
...
setTimeout(()=>{
    ...
    if (localStorage["source_local"] == null && localStorage["source"] == null) localStorage.setItem("source", default_opml);
...
let load_feeds = function(data) {
    ...
    //clean source feed
    for(let p = 0; p < feed_download_list.length; p++)if (source_url_cleaner.includes(feed_download_list[p].url) == false) feed_download_list.splice(p, 1);
};
//////////////////////////////
//download content////
//////////////////////////////
let rss_fetcher = function(param_url, param_limit, param_channel, param_category) {
    ...
    xhttp.onload = function() {
            ...
            if (el.length > 0) for(let i = 0; i < param_limit; i++){
                ...
                if (item_media == "podcast") {
                    if (el[i].getElementsByTagNameNS("*", "duration").length > 0) {
                        console.log(el[i].getElementsByTagNameNS("*", "duration")[0].textContent);
                        item_duration = el[i].getElementsByTagNameNS("*", "duration")[0].textContent;
                    }
                }
                //check valid date
            ...
            if (el.length > 0) for(let i1 = 0; i1 < param_limit; i1++){
                ...
                if (item_media == "podcast") {
                    if (el[i1].getElementsByTagNameNS("*", "duration").length > 0) {
                        console.log(el[i1].getElementsByTagNameNS("*", "duration")[0].textContent);
                        item_duration = el[i1].getElementsByTagNameNS("*", "duration")[0].textContent;
                    }
                }
                startlistened = "";
        ...
        if (xhttp.status === 301) {
            ...
            rss_fetcher(xhttp.getResponseHeader("Location"), param_limit, param_channel, param_channel);
        }
    ...
    function loadEnd(e) {
        ...
        if (k < feed_download_list.length - 1) {
            document.getElementById("intro-message").innerText = "loading data";
            k++;
            rss_fetcher(feed_download_list[k].url, feed_download_list[k].amount, feed_download_list[k].channel, feed_download_list[k].channel);
        }
    }
};
...
let filter_data = function(cat) {
    heroArray.length = 0;
    let index = 0;
    for(let i = 0; i < content_arr.length; i++)if (content_arr[i].category == cat) {
        index++;
...
function build() {
    ...
    tabs();
    clean_localstorage();
    _helperJs.bottom_bar("<img src='assets/icons/option.svg'>", "", "<img src='assets/icons/list.svg'>");
...
let mark_as_read = function(un_read) {
    ...
    if (un_read == false) {
        ...
        document.querySelector("[data-id ='" + status.active_element_id + "']").setAttribute("data-read", "not-read");
        let test = [];
        ...
        read_elem = test;
        _helperJs.side_toaster("article marked as not read", 2000);
    }
};
let sort_tab = function(type) {
    ...
    //sort
    if (type == "string") _helperJs.sort_array(heroArray, "title", "string");
    if (type == "number") _helperJs.sort_array(heroArray, "dateunix", "number");
    //build html
    renderHello(heroArray);
    show_article_list();
    _helperJs.side_toaster("sorted", 2000);
};
////////////////////////
//NAVIGATION
/////////////////////////
...
function nav(move) {
    ...
    if (document.activeElement.classList.contains("input-parent")) _helperJs.bottom_bar("", "", "");
...
let sleep_mode = function() {
    let st = _settingsJs.setting.sleep_time;
    st = st * 60000;
...
let show_article = function() {
    ...
    if (document.activeElement.getAttribute("data-media") == "podcast") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", "", "");
    if (document.activeElement.getAttribute("data-media") == "video") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", "", "");
    if (document.activeElement.getAttribute("data-media") == "rss") _helperJs.bottom_bar("<img src='assets/icons/E24F.svg'>", "", "");
    if (document.activeElement.getAttribute("data-media") == "youtube") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", "", "");
    ...
    if (document.activeElement.getAttribute("data-media") == "video") {
        ...
        video_player.onplaying = function() {
            ...
            video_time = setInterval(function() {
                ...
                if (video_status == "playing") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(t), "");
                if (video_status == "paused") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(t), "");
            }, 1000);
        };
        video_player.onpause = function() {
            document.getElementById("message").style.top = "-1000px";
            video_status = "paused";
            _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(t), "");
        };
        return;
    }
    //youtube
    if (document.activeElement.getAttribute("data-media") == "youtube") {
        ...
        function onPlayerStateChange(event) {
            ...
            youtube_time = setInterval(function() {
                ...
                document.getElementById("progress-bar").style.display = "block";
                document.querySelector("div#progress-bar div").style.width = percent + "%";
                if (youtube_status == "playing") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(t), "");
                if (youtube_status == "paused") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(t), "");
            }, 1000);
        }
...
let show_feed_download_list = function() {
    tab_index = 0;
    _helperJs.bottom_bar("", "<img src='assets/icons/pencil.svg'>", "");
...
let start_options = function() {
    ...
    if (document.activeElement.getAttribute("data-function") == "reload") reload();
    if (document.activeElement.getAttribute("data-function") == "download-list") show_feed_download_list();
    if (document.activeElement.getAttribute("data-function") == "sort-by-name") sort_tab("string");
    if (document.activeElement.getAttribute("data-function") == "sort-by-date") sort_tab("number");
...
qr_listener.addEventListener("focus", (event)=>{
    _helperJs.bottom_bar("<img src='assets/icons/E1D8.svg'>", "", "");
    qrscan = true;
});
...
document.querySelector("#source-local").addEventListener("change", (event)=>{
    localStorage.setItem("source_local", undefined.value);
    ...
    switch(param.key){
        ...
        case "Alt":
            ...
            if (status.window_status == "scan") {
                status.window_status = "settings";
                _scanJs.stop_scan(stop_scan_callback);
                break;
            }
            if (status.window_status == "settings") {
                show_article_list();
...
(function(global, factory) {
   ...
   */ var _createTrustedTypesPolicy = function _createTrustedTypesPolicy(trustedTypes, document) {
     ...
     */ /* allowed element names */ var ALLOWED_TAGS = null;
        ...
        }, [].concat(_toConsumableArray$1(html), _toConsumableArray$1(svg), _toConsumableArray$1(svgFilters), _toConsumableArray$1(mathMl), _toConsumableArray$1(text)));
        /* Allowed attribute names */ var ALLOWED_ATTR = null;
        var DEFAULT_ALLOWED_ATTR = addToSet({
        }, [].concat(_toConsumableArray$1(html$1), _toConsumableArray$1(svg$1), _toConsumableArray$1(mathMl$1), _toConsumableArray$1(xml)));
        /*
     ...
     */ // eslint-disable-next-line complexity
        var _parseConfig = function _parseConfig(cfg) {
            ...
            /* Parse profile info */ if (USE_PROFILES) {
                ...
                }, [].concat(_toConsumableArray$1(text)));
                ALLOWED_ATTR = [];
...
player.addEventListener("play", (event)=>{
    _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", duration, "");
});
...
player.addEventListener("playing", (event)=>{
    ...
    getduration = setInterval(function() {
        if (!player.paused) {
            ...
            if (_appJs.status.window_status == "audio-player") _helperJs.bottom_bar("<img src='assets/icons/23EF.svg'>", toTime(time), "");
        }
    }, 1000);
});
player.onended = function() {
    ...
    listened_elem.push(_appJs.status.active_audio_element_id);
...
},{"../../app.js":"20BJq","./helper.js":"db1Xp","../../assets/js/translations.js":"3MYwo","@parcel/transformer-js/src/esmodule-helpers.js":"j7FRh"}]},["8ov0T","20BJq"], "20BJq", "parcelRequire94c2")
